...
[
  {
    ...
    "story": "Ad-impression logs, ad-platform campaign feeds, and web-analytics events land in a governed lakehouse via Lakeflow. A standardized batch workflow trains XGBoost CTR-regression candidates with MLflow (every run/param/metric/feature-set logged), registers the winner in Unity Catalog with a @champion alias, and batch-scores every segment into a gold predictions table that AI/BI dashboards and a Genie Agent read — reached by the business through Genie One, all governed by Unity Catalog.",
    "options": {
      "trademarkLogos": true
    },
    "columns": [
    ...
    "nodes": [
      ...
      },
      {
        ...
        "ai_reasoning": "the 2-page CTR Optimization dashboard — predicted-vs-actual divergence, $242K wasted-spend KPI, champion vs stale lift"
      },
      {
        "id": "db-platform",
        "type": "db-platform",
      ...
      },
      {
        ...
        "ai_reasoning": "the one data-layer block: Lakeflow ingest + bronze→silver→gold SDP, built/maintained by Genie Code (Genie Code beat is built into this block's footer — no separate tile)"
      },
      {
        "id": "model-training",
        "type": "model-training",
      ...
      },
      {
        ...
        "ai_reasoning": "proper versioning + governance — the winning run is registered as a new UC version and promoted @champion (stale baseline stays as an older version for the before/after)"
      }
    ],
    "edges": [
      {
