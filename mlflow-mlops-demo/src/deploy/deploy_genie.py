...
"""
Deploy the NorthPeak CTR Optimization Genie Space — DAB setup-job task.

Loads the committed Genie space JSON (curated questions + curated SQLs + story
instructions for the CTR-drift investigation), substitutes the catalog/schema,
...
GOTCHAS — proven the hard way; do NOT "simplify" these away:
  ...
  • LOAD-FROM-JSON: the curated questions/SQLs are large; keep them in a JSON
    file. Substitute catalog.schema with a literal string-replace (the same
    qualifier appears in table identifiers AND inside SQL bodies).
...
# The space's display title (used for idempotent lookup) + its description.
SPACE_TITLE = "NorthPeak CTR Optimization Analytics"
SPACE_DESCRIPTION = (
    "Ask about NorthPeak Goods paid-acquisition CTR: a campaign mix shift 3 weeks "
    "ago moved budget to Social + Display, their CTR collapsed, the stale model "
    "over-bid (~$242K wasted spend), and a retrained champion model recovered "
    "accuracy. Walk the suggested questions in order on a cold start."
)

# The qualifier baked into the committed genie_space.json (the catalog.schema the
# JSON was authored against). We literal-replace this with the deployed
# catalog.schema. Keep in sync with src/genie/genie_space.json.
SRC_QUALIFIER = "solution_builder.demo_mlflow_logged_ctr_optimization"
...
# Substitute the authored catalog.schema with the deployed one.
