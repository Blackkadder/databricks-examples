...
does the full raw -> silver -> gold layering itself, all in Spark. Story: a
campaign mix shift ~21 days ago moved budget from Search to Social + Display;
those prospecting/awareness segments' true CTR collapsed from ~2.4% toward
~1.0%, but the stale production model still predicts the old rate ->
over-bidding -> ~$242K wasted spend over 3 weeks.
...
Runtime: on a Databricks job the notebook `spark` is used directly; locally it
falls back to a databricks-connect serverless session. Spark-native
...
# On a Databricks job `spark` is already bound; only build a databricks-connect
# session for local runs (import is lazy so the job runtime needs no extra dep).
try:
    spark  # noqa: F821
except NameError:
...
# Drift override: for drift campaigns in the drift window, ramp CTR down to ×0.40
...
imp = (
    ...
    # CPM billing (cost per impression served) — how awareness / display / social
    # inventory is actually bought, so spend concentrates on the high-volume
    # Social+Display awareness campaigns the mix shift tripled. Each synthetic row
    # is a sampled block of real impressions, so the per-row cost is a scaled CPM
    # (channel-dependent; awareness inventory is expensive relative to its CTR).
    .withColumn("cpm_row",
